...
export interface StrapiBaseCollectionController<T extends CollectionEntity> {
...
export interface StrapiBaseSingleTypeController<T extends SingleTypeEntity> {
  find(ctx: StrapiAppContext): Promise<T>;
  update(ctx: StrapiAppContext): Promise<T>;
  delete(ctx: StrapiAppContext): Promise<T>;
}

export interface StrapiBaseCollectionService<T extends CollectionEntity> {
  find(
  ...
  create(data: Partial<Omit<T, "id">>, options?: { files: any }): Promise<T>;
  update(
    params: StrapiQueryFilters<T>,
...
export interface StrapiBaseSingleTypeService<T extends SingleTypeEntity> {
  find(
    params?: StrapiQueryFilters<T>,
    populate?: StrapiQueryPopulate
  ): Promise<T>;
